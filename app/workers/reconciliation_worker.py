...
class ReconciliationWorker:
    ...
    async def _reconcile_provider_databases(self, provider: Provider):
        ...
        for db in databases:
            # Check if we should stop
            if not self.running:
                logger.info("reconciliation_stopped_during_provider_execution")
                return
    ...
    async def _reconcile_database(self, db: Database, provider: Provider):
        ...
        try:
            ...
            try:
                ...
                if stuck_pods > 0:
                    logger.info(
                        "stuck_pods_deleted_during_reconciliation",
                        database_id=db.id,
                        stuck_pods_count=stuck_pods,
                    )
            except Exception as e:
                # Log but don't fail reconciliation if stuck pod check fails
                logger.warning(
                    "stuck_pod_check_failed_during_reconciliation",
                    database_id=db.id,
                    error=str(e),
                )
    ...
    async def _create_reconciliation_operation(
        ...
        try:
            ...
            if existing:
                logger.info(
                    "operation_already_exists_skipping",
                    database_id=db.id,
...
async def main():
    ...
    async def monitor_shutdown():
        ...
        await worker.stop()
        # Cancel the worker's start() task to force exit
        # This ensures immediate shutdown even if worker is stuck
