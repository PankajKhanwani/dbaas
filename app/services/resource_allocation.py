...
class ResourceAllocationService:
    ...
    async def allocate_resources(
        ...
        Uses the Provider's atomic allocation method with optimistic locking.
        ...
        # Try atomic allocation (includes capacity check and retry logic)
        ...
        if not success:
            # Allocation failed (either out of capacity or max retries)
            ...
            raise ResourceAllocationError(
                f"Failed to allocate resources on provider {provider_id}. "
        ...
        # Return updated provider (refresh from DB to get latest state)
    ...
    async def deallocate_resources(
        ...
        Uses the Provider's atomic deallocation method with optimistic locking.
        ...
        Raises:
            ProviderNotFoundError: If provider doesn't exist
            ResourceAllocationError: If deallocation fails
        """
        provider = await Provider.find_one(Provider.id == provider_id)
        ...
        # Try atomic deallocation (includes retry logic)
        success = await provider.deallocate_resources_atomic(
            cpu_cores=cpu_cores,
            memory_gb=memory_gb,
            storage_gb=storage_gb,
            max_retries=ResourceAllocationService.MAX_RETRIES
        ...
        if not success:
            raise ResourceAllocationError(
                f"Failed to deallocate resources from provider {provider_id} after {ResourceAllocationService.MAX_RETRIES} attempts"
            )
